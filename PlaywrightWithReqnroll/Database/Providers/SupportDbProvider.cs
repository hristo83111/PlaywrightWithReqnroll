using Dapper;
using PlaywrightWithReqnroll.Configuration;
using PlaywrightWithReqnroll.Models;

namespace PlaywrightWithReqnroll.Database.Providers;

/// <summary>
/// Provides database operations for the Support database.
/// </summary>
public class SupportDbProvider(TestSettings testSettings)
    : BaseDbProvider(testSettings.ConnectionStrings.SupportDb)
{
    /// <summary>
    /// Asynchronously retrieves the first <see cref="LoginCredentials"/> record from the Support table.
    /// </summary>
    /// <returns>
    /// A task that represents the asynchronous operation. The task result contains the first <see cref="LoginCredentials"/> found, or <c>null</c> if no record exists.
    /// </returns>
    public async Task<LoginCredentials?> GetSupportAsync()
    {
        var result = await Connection.QueryAsync<LoginCredentials>(
            "SELECT * FROM Support");
        return result.FirstOrDefault();
    }
}
